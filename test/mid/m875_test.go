...
func minEatingSpeed(piles []int, h int) int {
	...
	if len(piles) == h || min == 0 {
		return min
	}

	for temp := min / 2; temp <= min && temp > 0; {
		if temp == min {
			break
		}
		cost := 0
		...
		fmt.Println("cost = ", cost, "min = ", min)
		// break
		if cost > h {
			temp = (temp + min + 1) / 2

			fmt.Println("###cost > h --- temp  = ", temp, "min = ", min, "cost = ", cost)
		} else {
			fmt.Println("###--- temp  = ", temp, "min = ", min, "cost = ", cost)
			min = temp
			temp /= 2
		}
	}
